/**
 * TimerControls.vue コンポーネントテスト（タスク 8.8）
 *
 * 検証内容:
...
// AudioService をモック化して、実際のビープ音（Web Audio API）が鳴らないようにする
...
// ---------------------------------------------------------------------------
// aria-label の付与（要件 8.2, 8.4）
//
// 要件 8.2: 全てのボタンにアクセシブルなラベル（aria-label）が付与されている
// 要件 8.4: ボタンには type="button" が指定されている
// ---------------------------------------------------------------------------

describe('TimerControls.vue — aria-label の付与（要件 8.2, 8.4）', () => {
  it('isRunning=false のとき、全ボタンに aria-label が付与されている', () => {
    const { container } = render(TimerControls, {
  ...
  it('isRunning=true のとき、全ボタンに aria-label が付与されている', () => {
    const { container } = render(TimerControls, {
  ...
  it('リセットボタンには常に aria-label="リセット" が付与されている', () => {
    for (const isRunning of [false, true]) {
      const { getByRole, unmount } = render(TimerControls, {
        props: { isRunning },
      });
  ...
  it('全ボタンに type="button" が指定されている（要件 8.4）', () => {
    for (const isRunning of [false, true]) {
      const { container, unmount } = render(TimerControls, {
        props: { isRunning },
      });
...
// ---------------------------------------------------------------------------
// notifyPhaseEnd — 通知許可の状態に応じた動作（要件 5.4）
//
// 要件 5.4: 通知を拒否した場合はサウンドのみで通知し、Notification は表示しない
// ---------------------------------------------------------------------------

describe('notifyPhaseEnd() — 通知許可の状態に応じた動作（要件 5.4）', () => {
  ...
  it('permission=granted のとき、サウンド再生に加えて Notification が表示される（要件 5.2 の対比確認）', () => {
